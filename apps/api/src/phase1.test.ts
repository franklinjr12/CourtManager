...
async function setup() {
  ...
  const coachRecord = await services.staff.create(owner, {
    name: 'Coach',
    email: 'coach@phase1.test',
    role: 'COACH',
    password: 'password',
  });
  ...
  return {
    ...
    court,
    customer,
    coachContext: { ...coach, userId: String(coachRecord.userId) },
  };
}
describe('Phase 1 operations', () => {
  it('enforces reservation attendance lifecycle and releases no-shows', async () => {
    ...
    const reservation = await services.reservations.create(owner, {
      ...
      startAt: '2027-03-01T18:00:00Z',
      endAt: '2027-03-01T19:00:00Z',
      source: 'STAFF',
    });
    ...
    await expect(
      services.reservations.transition(
        owner,
        String(reservation.reservationId),
        'NO_SHOW',
      ),
    ...
    const noShow = await services.reservations.create(owner, {
      courtId: court.courtId,
      customerId: customer.customerId,
      startAt: '2027-03-01T19:00:00Z',
      endAt: '2027-03-01T20:00:00Z',
      source: 'STAFF',
    });
    await services.reservations.transition(
      owner,
    ...
    await expect(
      services.reservations.create(owner, {
        courtId: court.courtId,
        customerId: customer.customerId,
        startAt: '2027-03-01T19:00:00Z',
        endAt: '2027-03-01T20:00:00Z',
  ...
  it('materializes class sessions, charges enrollment and records roster attendance', async () => {
    ...
    const cls = await services.classes.create(owner, {
      ...
      sport: 'Tennis',
      type: 'GROUP',
      coachId: String(coachContext.userId),
    ...
    const enrollment = await services.classes.enroll(
      owner,
      String(cls.classId),
      String(customer.customerId),
    );
    expect(enrollment.status).toBe('ACTIVE');
    const roster = await services.classes.roster(
      coachContext,
      `${String(cls.classId)}-2027-03-02`,
    );
    ...
    await services.classes.completeSession(
      coachContext,
      `${String(cls.classId)}-2027-03-02`,
    );
    expect(
      (
        await services.classes.roster(
          coachContext,
          `${String(cls.classId)}-2027-03-02`,
        )
    ...
    expect(
      (await services.charges.list(owner)).some(
        (charge) =>
          (charge as Record<string, unknown>)['classSessionId'] ===
          `${String(cls.classId)}-2027-03-02`,
      ),
    ).toBe(true);
  });
  it('provides tenant-safe balances and Today aggregation', async () => {
    ...
    const reservation = await services.reservations.create(owner, {
      ...
      startAt: '2027-03-03T18:00:00Z',
      endAt: '2027-03-03T19:00:00Z',
      expectedAmount: 100,
      source: 'STAFF',
    });
    await services.payments.create(owner, {
      ...
      amount: 25,
      method: 'PIX',
      paidAt: '2027-03-03T18:00:00Z',
    });
    expect((await services.finance.balances(owner))[0]).toMatchObject({
      outstanding: 75,
    });
    const today = await services.today.get(
      owner,
      new Date('2027-03-03T18:30:00Z'),
    );
