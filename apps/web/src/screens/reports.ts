...
export async function reports() {
  await shell(async () => {
    const now = new Date(), from = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, '0')}-01`, to = now.toISOString().slice(0, 10);
    const data = await request<Record<string, unknown>>(`/reports/operations?from=${from}&to=${to}`);
    const utilization = (data.utilization as Array<Record<string, unknown>> | undefined) ?? [];
