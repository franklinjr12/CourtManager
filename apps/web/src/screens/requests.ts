...
function openConfirmRequest(item: RequestItem, customers: Customer[]) {
  openModal(
    ...
    `<form id="confirm-request-form" class="form-grid"><p class="full">${escapeText(item.customerName)} Ã‚Â· ${escapeText(dateValue(item.requestedStartAt))} ${escapeText(timeValue(item.requestedStartAt))}</p><label class="full">${t('common.customer')}<select name="customerId"><option value="">${t('requests.createCustomer')}</option>${customerOptions(customers, item.linkedCustomerId)}</select></label><p class="form-error" role="alert"></p><div class="form-actions full"><button type="button" class="button" data-close>${t('common.cancel')}</button><button class="button primary">${t('requests.confirmButton')}</button></div></form>`,
  );
