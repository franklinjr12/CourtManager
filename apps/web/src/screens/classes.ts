...
export async function classes() {
  ...
  const customers =
    session()?.user.role === 'COACH'
      ? []
      : await request<Customer[]>('/customers?limit=100');
  const coaches =
    session()?.user.role === 'COACH'
      ? []
      : await request<Array<{ userId: string; name: string }>>('/coaches');
  await shell(async () => {
  ...
  if (org.features.classes)
    setTimeout(() => {
      if (session()?.user.role === 'COACH') {
        ...
        screen()
          ?.querySelectorAll('[data-enroll-class]')
...
function classForm(
  courts: Court[],
  _coaches: Array<{ userId: string; name: string }>,
) {
...
function openClassModal(
  courts: Court[],
  coaches: Array<{ userId: string; name: string }>,
) {
  ...
  if (form) {
    form.insertAdjacentHTML(
      'afterbegin',
      `<label>${t('staff.role')}<select name="coachId" required>${coaches.map((coach) => `<option value="${escapeText(coach.userId)}">${escapeText(coach.name)}</option>`).join('')}</select></label>`,
    );
    const price = form.elements.namedItem('price');
    if (price instanceof HTMLElement)
      price.setAttribute('name', 'pricePerParticipant');
  }
