...
type TodayData = {
  ...
  arrivalsNextHour: Record<string, unknown>[];
  overdueCheckIns: Array<{ reservationId: string; label: string }>;
  pendingRequests: Record<string, unknown>[];
  attention: Array<{ type: string; label: string; reservationId?: string }>;
};
export async function today() {
  await shell(async () => {
    const data = await request<TodayData>('/today');
    setTimeout(() => wireToday(data), 0);
    const courtRows = data.courts
      .map(
        (item) =>
...
function wireToday(data: TodayData) {
  app.querySelectorAll<HTMLElement>('[data-check-in]').forEach((button) =>
    button.addEventListener('click', async () => {
      try {
        await request(`/reservations/${button.dataset.checkIn}/check-in`, {
          method: 'POST',
        });
        await renderRoute();
      } catch (error) {
