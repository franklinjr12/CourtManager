...
export async function reservations() {
  ...
  rows?.forEach((row, index) => {
    ...
    if (reservation) {
      ...
      if (cells[3])
        cells[3].textContent = `${reservationStatusLabel(reservation.status)} Â· ${paymentStatusLabel(reservation.paymentStatus ?? 'UNPAID')}`;
    }
  });
}
