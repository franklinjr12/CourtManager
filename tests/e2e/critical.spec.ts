...
async function api(page: Page, path: string, init: RequestInit = {}) {
  return page.evaluate(
    async ({ apiBase, path, init }) => {
      const current = JSON.parse(localStorage.getItem('court-manager-session') ?? 'null');
...
test('owner can manage courts from Settings', async ({ page }) => {
  ...
  const court = courts.body.data.find((item: { name: string }) => item.name === unique);
  expect(court).toBeTruthy();
  await row.getByRole('button', { name: 'Edit' }).click();
  await page.getByRole('dialog').getByLabel('Name').fill(edited);
  await page.getByRole('dialog').getByRole('button', { name: 'Save changes' }).click();
  await expect(page.locator('.list-row').filter({ hasText: edited })).toBeVisible();
  ...
  if (court) expect((await api(page, `/courts/${court.courtId}`)).status).toBe(404);
});
...
test('new reservation opens, supports quick customer creation, and creates a booking', async ({
  ...
  const dialog = page.getByRole('dialog');
  await expect(dialog.getByRole('heading', { name: 'New reservation' })).toBeVisible();
  ...
  await expect(
    dialog.locator('select[name="customerId"] option').filter({ hasText: unique }),
  ).toHaveCount(1);
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  const date = tomorrow.toISOString().slice(0, 10);
  await dialog.getByLabel('Date').fill(date);
  await expect(dialog.locator('select[name="startTime"] option').first()).not.toHaveText(
    'Loading…',
  );
  ...
  const customers = await api(page, `/customers?search=${encodeURIComponent(unique)}`);
  ...
  if (reservation)
    await api(page, `/reservations/${reservation.reservationId}/cancel`, { method: 'POST' });
  await api(page, `/customers/${customer.customerId}/archive`, { method: 'POST' });
});

test('mobile schedule keeps actions usable and modal closes with Escape', async ({ page }) => {
...
test('owner operational screens expose workflows instead of placeholders', async ({ page }) => {
  await login(page);
  await page.goto('/settings');
  await expect(
